...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        """
        def average(item: dict[str, Any]) -> float | None:
            values = item.get("scores", {}).values()
            numeric = [float(value) for value in values if isinstance(value, (int, float))]
            return sum(numeric) / len(numeric) if numeric else None

        averages = [value for item in scores_batch if (value := average(item)) is not None]
        overall_average = sum(averages) / len(averages) if averages else 0.5

        positional_bias = False
        if len(averages) >= 2:
            first_position = averages[0::2]
            second_position = averages[1::2]
        ...
        return {
            ...
            "leniency_bias": bool(averages) and overall_average > 0.8,
            "severity_bias": bool(averages) and overall_average < 0.3,
        }
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        for result in results:
            if not result.passed:
                failure_name = result.failure_type or "unknown"
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def metric_average(results: list[EvalResult], name: str) -> float:
            if not results:
                return 0.0
            return sum(getattr(result, name) for result in results) / len(results)

        metrics = ("faithfulness", "relevance", "completeness")
        new_averages = {name: metric_average(new_results, name) for name in metrics}
        baseline_averages = {
            name: metric_average(baseline_results, name) for name in metrics
        }
        regressions = [
        ...
        report: dict[str, Any] = {
            f"new_avg_{name}": new_averages[name] for name in metrics
        }
        report.update({
            f"baseline_avg_{name}": baseline_averages[name] for name in metrics
        })
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        for failure in failures:
            failure_name = failure.failure_type or "unknown"
            categories[failure_name] = categories.get(failure_name, 0) + 1
        return categories
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = (
                ...
                else "Review this failure and define a targeted corrective action"
            ).replace("|", "\\|")
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        actions = {
            "hallucination": "Implement a grounding check and require evidence for unsupported claims",
            "irrelevant": "Add intent-focused few-shot examples and tighten the answer relevance prompt",
            "incomplete": "Improve retrieval coverage and add an expected-information checklist to generation",
            "off_topic": "Improve intent detection and route out-of-scope questions explicitly",
            "refusal": "Calibrate guardrails with allowed examples to reduce unnecessary refusals",
            "unknown": "Review unlabeled failures and assign a consistent failure taxonomy",
        }
        suggestions = [
        ...
        fallback_actions = [
            "Add representative failures to the versioned regression dataset",
            "Track answer and retrieval metrics separately to isolate the failing stage",
            "Run a targeted evaluation after each prompt, retriever, or guardrail change",
        ]
